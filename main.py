...
async def webhook(req: Request):
    ...
    if "message" in update:
        ...
        if chat_id == ADMIN_ID:
            ...
            # ----- 3) /sendaccess (single, clickable, previews OFF, with lock) -----
            if text.startswith("/sendaccess"):
                ...
                try:
                    # Build clickable display from stored label + url
                    ...
                    for uid in ids:
                        try:
                            ...
                            await tg("sendMessage", {
                                ...
                                "text": msg_text,
                                "parse_mode": "HTML",
                                "disable_web_page_preview": True  # previews OFF
                            })
                            sent += 1
                            await asyncio.sleep(0.03)
                        except Exception:
            ...
            # manual unlock helper
            if text.startswith("/unlocksendaccess"):
                await r_set("lock:sendaccess", "0")
                await tg("sendMessage", {"chat_id": chat_id, "text": "sendaccess lock cleared. You can run /sendaccess now."})
                return {"ok": True}
...
# THIS LINE keeps your media-aware UI working
send_ui = send_ui_v2
