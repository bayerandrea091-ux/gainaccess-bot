...
async def webhook(req: Request):
    ...
    if "callback_query" in update:
        ...
        if data == "access":
            # increment fake tap counter up to 3 (never touching real shares)
            real_n = int((await r_get(f"shares:{uid}") or "0"))
            tap_n = int((await r_get(f"taps:{uid}") or "0"))
            if real_n < 3 and tap_n < 3:
                tap_n += 1
                await r_set(f"taps:{uid}", str(tap_n))
            display_n = max(real_n, min(tap_n, 3))

            # popup with current displayed progress
            await tg("answerCallbackQuery", {
                "callback_query_id": cb["id"],
                "show_alert": True,
                "text": T(uid, "shares", n=display_n, goal=GOAL)
            })

            # correct hint depending on threshold
            hint_key = "access_hint" if display_n < 3 else "access_hint_stuck"
            await tg("sendMessage", {
                "chat_id": uid,
                "text": T(uid, hint_key),
                "reply_markup": {
                    "inline_keyboard": [[
                        {"text": f"Share again {display_n}/{GOAL}", "url": f"https://t.me/share/url?url={quote_plus(SHARE_URL)}"}
                    ]]
                }
            })

            # re-paint UI so all buttons reflect n=display_n (this is what makes 0→1→2→3 visible)
            try:
                await send_ui(uid, shares_n=display_n)
            except:
                pass
...
async def share_redirect(req: Request):
    try:
        uid = int(req.query_params.get("u", "0"))
    except:
        uid = 0

    if uid:
        # figure out current display value: max(real shares, taps capped at 3)
        try:
            real_n = int((await r_get(f"shares:{uid}") or "0"))
        except:
            real_n = 0
        ...
        # edit the last UI message in place (if we have its message_id)
        mid = await r_get(f"ui:{uid}:mid")
        if mid:
            try:
                await tg("editMessageReplyMarkup", {
                    "chat_id": uid,
                    "message_id": int(mid),
                    "reply_markup": keyboard(uid, display_n)
                })
            except:
                pass

    # 302 to native Telegram share sheet
