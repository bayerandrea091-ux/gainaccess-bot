...
async def webhook(req: Request):
    ...
    # build leaderboard lines
        ...
        if chat_id == ADMIN_ID:
            ...
            # ===== ADDED ADMIN COMMAND: /setchannelid =====
            if text.startswith("/setchannelid"):
                payload = text[len("/setchannelid"):].strip()
                if not payload:
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Usage: /setchannelid <chat_id_or_username>"})
                    return {"ok": True}
                # store raw id/username
                await r_set("channel:id", payload)
                # try to fetch title using getChat (requires bot is admin/has access)
                try:
                    resp = await tg("getChat", {"chat_id": payload})
                    if isinstance(resp, dict) and resp.get("ok") and "result" in resp:
                        title = resp["result"].get("title")
                        if title:
                            await r_set("channel:title", title)
                            await tg("sendMessage", {"chat_id": chat_id, "text": f"Channel id stored and title fetched: {title}"})
                        else:
                            await tg("sendMessage", {"chat_id": chat_id, "text": "Channel id stored. Could not fetch a title (maybe private or bot lacks permission)."})
                    else:
                        await tg("sendMessage", {"chat_id": chat_id, "text": "Channel id stored. Could not fetch title (getChat failed)."})
                except Exception:
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Channel id stored. Could not fetch title (exception)."})
                return {"ok": True}

            # ===== ADDED ADMIN COMMAND: /setchannellabel =====
            # Usage: /setchannellabel Label Text
            # Or:   /setchannellabel Label Text | https://t.me/xxxxx
            if text.startswith("/setchannellabel"):
                payload = text[len("/setchannellabel"):].strip()
                if not payload:
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Usage: /setchannellabel Label Text [| optional_url]"})
                    return {"ok": True}
                parts = [p.strip() for p in payload.split("|", 1)]
                label = parts[0]
                await r_set("channel:label", label)
                if len(parts) > 1 and parts[1]:
                    await r_set("channel:url", parts[1])
                await tg("sendMessage", {"chat_id": chat_id, "text": "Channel label (and optional URL) saved."})
                return {"ok": True}

            # ===== ADDED ADMIN COMMAND: /sendaccess =====
            # Broadcasts the short access_required message to all subscribers
            if text.startswith("/sendaccess"):
                # pick channel display preference: label > title > env URL > fallback
                label = await r_get("channel:label")
                title = await r_get("channel:title")
                url = await r_get("channel:url")
                if label:
                    display = label
                elif title:
                    display = title
                    ...
                    display = "(the channel)"
                msg_template = T(chat_id, "access_required", channel=display)
                ids = await r_smembers("subs")
                sent = 0
                for uid in ids:
                    try:
                        await tg("sendMessage", {"chat_id": int(uid), "text": msg_template, "parse_mode": "HTML"})
                        sent += 1
                        await asyncio.sleep(0.03)
                    except Exception:
                        pass
