...
import httpx
import random  # <-- PATCH 1: for /tip rotation
...
# ---- PATCH 1b: rotating tips ----
TIPS = [
    "Try again if a link is busy.",
    "Use a VPN if Telegram feels slow.",
    "Tap SHARE multiple times to reach the goal.",
    "Pin this chat so you don’t lose it.",
    "If buttons don’t open, update Telegram and try again.",
]
...
# ---- PATCH 2: needed by polls to store votes as a hash ----
async def r_hset(key: str, field: str, value: str) -> None:
    if not UPSTASH_URL or not UPSTASH_TOKEN: 
        return
    async with httpx.AsyncClient(timeout=15) as c:
        r = await c.post(
            f"{UPSTASH_URL}/hset/{key}/{field}/{httpx.QueryParams({'v': value})['v']}",
            headers=_auth()
        )
        r.raise_for_status()
...
async def webhook(req: Request):
    ...
    if "message" in update:
        ...
        if text.startswith("/tip"):
            await tg("sendMessage", {"chat_id": chat_id, "text": f"Tip: {random.choice(TIPS)}"})
            return {"ok": True}
    ...
    if "callback_query" in update:
        ...
        if data == "access":
            ...
            # ---- PATCH 3: send “Share again” button under the chat
            await tg("sendMessage", {
                "chat_id": uid,
                "text": "Not there yet? Tap SHARE again to keep going ⤵️",
                "reply_markup": {
                    "inline_keyboard": [[
            ...
            # ---- PATCH 2 use hash to store votes
