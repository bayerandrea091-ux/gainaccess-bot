...
async def webhook(req: Request):
    ...
    if "callback_query" in update:
        ...
        if data == "access":
            # CHANGED: implement tap-count (0→3) and show updated hint + re-render UI with n
            ...
            await tg("sendMessage", {
                ...
                "reply_markup": {
                    "inline_keyboard": [[
                        {"text": f"Share again {display_n}/{GOAL}", "url": f"https://t.me/share/url?url={quote_plus(SHARE_URL)}"}  # (update)
                    ]]
                }
            })
            # Repaint the main UI so the top buttons reflect n={display_n}
            try:
