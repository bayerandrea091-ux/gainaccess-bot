...
# NEW: real lock helpers (SETNX/EXPIRE/DEL)
...
async def webhook(req: Request):
    ...
    if "message" in update:
        ...
        if chat_id == ADMIN_ID:
            ...
            # ===== NEW: /sendaccess with SETNX lock + TTL + background task =====
            if text == "/sendaccess":
                # only allow from ADMIN in private chat
                if not (chat_id == ADMIN_ID and is_private):
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Only the admin can run /sendaccess in a private chat."})
                    return {"ok": True}

                # acquire distributed lock (prevents retries & double runs across processes)
                locked = await r_setnx("lock:sendaccess", "1")
                if locked != 1:
                    await tg("sendMessage", {"chat_id": chat_id, "text": "sendaccess is already running — try again later."})
                    return {"ok": True}

                # auto-release after 10 minutes even if the process dies
                await r_expire("lock:sendaccess", 600)

                # quick ACK so Telegram won't retry this update
                await tg("sendMessage", {"chat_id": chat_id, "text": "Sending access message in the background… You'll get a summary."})

                async def _broadcast_sendaccess(admin_chat_id: int):
                    try:
                        ...
                        for uid in ids:
                            try:
                                ...
                                await tg("sendMessage", {
                                    ...
                                    "text": msg_text,
                                    "parse_mode": "HTML",
                                    "disable_web_page_preview": True  # keep previews OFF
                                })
                                sent += 1
                                await asyncio.sleep(0.03)
                            except Exception:
                        ...
                        await tg("sendMessage", {"chat_id": admin_chat_id, "text": f"Access message sent to {sent} users."})
                    finally:
                        # release the lock
                        await r_del("lock:sendaccess")

                asyncio.create_task(_broadcast_sendaccess(chat_id))
                return {"ok": True}
