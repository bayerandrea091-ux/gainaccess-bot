...
async def webhook(req: Request):
    ...
    # build leaderboard lines
        ...
        if chat_id == ADMIN_ID:
            ...
            # ============================
            # UPDATE 2) /setchannelid admin command
            # Usage: /setchannelid <channel_id_or_username>
            # Stores channel:id and tries to fetch channel title via getChat.
            if text.startswith("/setchannelid"):
                payload = text[len("/setchannelid"):].strip()
                if not payload:
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Usage: /setchannelid @channelusername or channel_id"})
                    return {"ok": True}
                try:
                    resp = await tg("getChat", {"chat_id": payload})
                    if resp.get("ok") and "result" in resp:
                        title = resp["result"].get("title") or resp["result"].get("username") or str(payload)
                        await r_set("channel:id", payload)
                        await r_set("channel:title", title)
                        await tg("sendMessage", {"chat_id": chat_id, "text": f"Channel saved: {title} ({payload})"})
                    else:
                        # still save the id but warn
                        await r_set("channel:id", payload)
                        await tg("sendMessage", {"chat_id": chat_id, "text": f"Saved id but couldn't fetch title for {payload}. You may set label with /setchannellabel."})
                except Exception:
                    await r_set("channel:id", payload)
                    await tg("sendMessage", {"chat_id": chat_id, "text": f"Saved id but getChat failed for {payload}."})
                return {"ok": True}

            # ============================
            # UPDATE 4) /setchannellabel admin command (fallback)
            # Usage examples:
            # /setchannellabel Channel Name
            # /setchannellabel Channel Name | https://t.me/joinchat/xxxx
            if text.startswith("/setchannellabel"):
                ...
                if url:
                    ...
                    await tg("sendMessage", {"chat_id": chat_id, "text": f"Channel label and URL saved: {label} | {url}"})
                else:
                    await tg("sendMessage", {"chat_id": chat_id, "text": f"Channel label saved: {label}"})
                return {"ok": True}

            # ============================
            # UPDATE 3) /sendaccess admin command
            # Broadcasts the access_required message to all subscribers.
            if text.startswith("/sendaccess"):
                # optional override: /sendaccess Some Label | optional_url
                override = text[len("/sendaccess"):].strip()
                if override:
                    parts = [p.strip() for p in override.split("|", 1)]
                    label = parts[0]
                    url = parts[1] if len(parts) > 1 else ""
                    channel_display = label
                else:
                    # choose best channel display available
                    channel_title = await r_get("channel:title")
                    channel_label = await r_get("channel:label")
                    channel_url = await r_get("channel:url")
                    channel_id = await r_get("channel:id")
                    if channel_title:
                        channel_display = channel_title
                    elif channel_label:
                        channel_display = channel_label
                    elif channel_url:
                        channel_display = channel_url
                    elif channel_id:
                ...
                for uid in ids:
                    try:
                        await tg("sendMessage", {
                            "chat_id": int(uid),
                            "text": T(int(uid), "access_required", channel=channel_display),
                            "parse_mode": "HTML",
