...
async def webhook(req: Request):
    ...
    if "message" in update:
        ...
        if text.startswith("/start") or text == "/menu":
            # NEW: reset this user before showing UI
            try:
                await r_set(f"shares:{chat_id}", "0")
                await r_set(f"taps:{chat_id}", "0")  # harmless if unused
            except:
                pass
            name = f"<a href='tg://user?id={chat_id}'>{first_name}</a>"
            await tg("sendMessage", {"chat_id": chat_id, "text": T(chat_id, "hi", name=name), "parse_mode": "HTML"})
        ...
        if is_admin(chat_id):  # CHANGED: multi-admin aware
            ...
            if text.startswith("/blast"):
                ...
                if not ids:
                    ...
                    # NEW: reset everyone before sending fresh UI
                    for uid in ids:
                        try:
    ...
    if "callback_query" in update:
        ...
        if data == "access":
            # NEW: Access button does NOT change counters or resend UI.
            # It only shows the shares popup and a single hint depending on real shares count.
            ...
            hint_key = "access_hint" if n < 3 else "access_hint_stuck" if n < GOAL else "access_hint"
            await tg("sendMessage", {
                "chat_id": uid,
                "text": T(uid, hint_key),
                "reply_markup": {
