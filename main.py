...
# --- Idempotency helpers ---
async def r_setnx(key: str, value: str) -> int:
    data = await _req("POST", f"setnx/{enc(key)}/{enc(value)}")
    try:
        return int(data.get("result", 0)) if isinstance(data, dict) else 0
    except Exception:
        return 0

async def r_expire(key: str, seconds: int):
    await _req("POST", f"expire/{enc(key)}/{seconds}")
...
# ====== Bulk helpers (add-only) ======
async def _with_lock(lock_key: str, ttl: int, runner: Callable[[], Awaitable[None]]) -> bool:
    """Return True if started; False if lock already held."""
    ok = await r_setnx(lock_key, "1")
    if ok != 1:
        return False
    await r_expire(lock_key, ttl)
    try:
        await runner()
    finally:
        # clear lock early
        await _req("POST", f"del/{enc(lock_key)}")
    return True

def fire_and_forget(lock_key: str, ttl: int, runner: Callable[[], Awaitable[None]]):
    async def _wrap():
        await _with_lock(lock_key, ttl, runner)
...
async def webhook(req: Request):
    ...
    # ---- GLOBAL: drop duplicate/retried updates (prevents all spam/replays) ----
    upd_id = update.get("update_id")
    if upd_id is not None:
        if (await r_setnx(f"upd:{upd_id}", "1")) != 1:
            return {"ok": True}
        await r_expire(f"upd:{upd_id}", 600)  # keep 10 minutes
    ...
    if "message" in update:
        ...
        if chat_id == ADMIN_ID:
            ...
            if text.startswith("/broadcast") and msg.get("reply_to_message"):
                # background copy of replied message to all
                async def _do_copy():
                ...
                if not await r_get("lock:broadcast_copy"):
                    fire_and_forget("lock:broadcast_copy", 900, _do_copy)
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Broadcast started (media copy)…"})
                else:
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Broadcast is already running — try again later."})
                return {"ok": True}
            ...
            if text.startswith("/senddaily"):
                ...
                if not await r_get("lock:senddaily"):
                    fire_and_forget("lock:senddaily", 900, _do_senddaily)
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Daily send started in background…"})
                else:
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Daily send already running — try again later."})
                return {"ok": True}
            ...
            if text.startswith("/drop"):
                ...
                if not await r_get("lock:drop"):
                    fire_and_forget("lock:drop", 900, _do_drop)
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Drop started in background…"})
                else:
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Drop is already running — try again later."})
                return {"ok": True}
            ...
            if text.startswith("/poll"):
                ...
                if not await r_get("lock:pollsend"):
                    fire_and_forget("lock:pollsend", 900, _do_poll)
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Poll distribution started…"})
                else:
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Poll send already running — try again later."})
                return {"ok": True}
            ...
            if text.startswith("/broadcast") and not msg.get("reply_to_message"):
                ...
                if not payload:
                    ...
                    if not await r_get("lock:broadcast_text"):
                        fire_and_forget("lock:broadcast_text", 900, _do_bc)
                        await tg("sendMessage", {"chat_id": chat_id, "text": "Broadcast started in background…"})
                    else:
                        await tg("sendMessage", {"chat_id": chat_id, "text": "Broadcast already running — try again later."})
                return {"ok": True}
            ...
            if text.startswith("/blast"):
                ...
                if not await r_get("lock:blast"):
                    fire_and_forget("lock:blast", 900, _do_blast)
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Blast started in background…"})
                else:
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Blast already running — try again later."})
                return {"ok": True}
            ...
            if text.startswith("/sendaccess"):
                async def _do_sendaccess():
                    ...
                    if label and ch_url:
                        ...
                        display = f"<a href='{ch_url}'>{safe_label}</a>"
                    elif ch_url:
                        display = ch_url
                    else:
                        # fallback to stored title/id or placeholder
                        title = await r_get("channel:title")
                        ch_id = await r_get("channel:id")
                        display = title or ch_id or "(the channel)"
                    ...
                    for uid in ids:
                        try:
                            ...
                            await tg("sendMessage", {
                                ...
                                "text": msg_text,
                                "parse_mode": "HTML",
                                "disable_web_page_preview": True  # no previews
                            })
                            sent += 1
                            await asyncio.sleep(0.03)
                        except Exception:
                ...
                if not await r_get("lock:sendaccess"):
                    fire_and_forget("lock:sendaccess", 900, _do_sendaccess)
                    await tg("sendMessage", {"chat_id": chat_id, "text": "Sending access message in background…"})
                else:
