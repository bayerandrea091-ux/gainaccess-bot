...
# --- ADDED (tiny helpers for /sendaccess lock) ---
async def r_setnx(key: str, value: str | int) -> int:
    """Set key only if it does not exist. Returns 1 if set, 0 if not."""
...
async def r_expire(key: str, seconds: int) -> int:
    """Set TTL on key. Returns 1 on success."""
    data = await _req("POST", f"expire/{enc(key)}/{seconds}")
    try:
        return int(data.get("result", 0)) if isinstance(data, dict) else 0
    except Exception:
        return 0
...
async def webhook(req: Request):
    ...
    if "message" in update:
        ...
        if chat_id == ADMIN_ID:
            ...
            # ----- 3) /sendaccess (on-demand; safe lock; clickable name) -----
            if text.startswith("/sendaccess"):
                # acquire lock atomically; if we can't, tell admin how to unlock
                got = await r_setnx("lock:sendaccess", "1")
                if got != 1:
                    await tg("sendMessage", {
                        "chat_id": chat_id,
                        "text": "sendaccess is already running — try again later.\nUse /unlocksendaccess if it seems stuck."
                    })
                    return {"ok": True}

                # lock auto-expires in 15 minutes in case of crash
                await r_expire("lock:sendaccess", 900)
                ...
                try:
                    ...
                    await _req("POST", f"del/{enc('lock:sendaccess')}")

            # manual unlock in case a past run crashed
