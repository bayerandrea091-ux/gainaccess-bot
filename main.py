...
# >>> UPDATE 1 (part A): optional UI media overrides <<<
UI_MEDIA_URL  = os.getenv("UI_MEDIA_URL", "").strip()
UI_MEDIA_TYPE = os.getenv("UI_MEDIA_TYPE", "").strip().lower()  # "", "photo", "video", "gif"
...
async def send_ui(chat_id: int, shares_n: int = 0):
    ...
    # >>> UPDATE 1 (part B): choose media kind (video/gif/photo) <<<
    media_url = (UI_MEDIA_URL or "").strip() or (IMAGE_URL or "").strip()
    media_type = UI_MEDIA_TYPE  # may be "", "video", "gif", "photo"
    if media_url:
        infer = ""
        low = media_url.lower()
        if low.endswith((".mp4", ".mov", ".m4v", ".webm")): infer = "video"
        elif low.endswith(".gif"): infer = "gif"
        else: infer = "photo"
        kind = media_type if media_type in ("video", "gif", "photo") else infer

        if kind == "video":
            await tg("sendVideo", {"chat_id": chat_id, "video": media_url, "caption": title, **kb})
            return
        if kind == "gif":
    ...
    # fallback if no media_url and no IMAGE_URL
...
async def webhook(req: Request):
    ...
    if "message" in update:
        ...
        if chat_id == ADMIN_ID:
            ...
            # >>> UPDATE 2: /broadcast supports replying to a media/message <<<
            if text.startswith("/broadcast"):
                ...
                if reply:
                    ...
                    for uid in ids:
                        try:
                            # copyMessage preserves any media/caption without downloading
                    ...
                    if not payload:
                        await tg("sendMessage", {"chat_id": chat_id, "text": "Usage: /broadcast Your message (or reply to a media message with /broadcast)"})
                    else:
                        ids = await r_smembers("subs")
                        sent = 0
                        for uid in ids:
